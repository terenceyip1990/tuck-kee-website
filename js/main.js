...
(function initCart(){
  ...
  if (placeBtn) {
    placeBtn.addEventListener('click', () => {
      ...
      if (!name || !phone) {
        if (errEl) { errEl.textContent = 'Please fill in your name and phone number.'; errEl.style.display = 'block'; }
        return;
      }
      ...
      const msg = `Hello Tuck Kee! I'd like to place an order 🙏%0A%0A${orderLines}%0A%0AEstimated Total%3A RM ${total.toFixed(2)}%0A%0AName%3A ${encodeURIComponent(name)}%0APhone%3A ${encodeURIComponent(phone)}${emailLine}%0A%0APlease confirm my order. Thank you!`;
